...
const HairCare = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const data = await getProducts({ category: 'Hair Care' });
        console.log('Hair Care API Response:', data);
        setProducts(data.data || []);
      } catch (error) {
        console.error('Error fetching hair care products:', error);
        toast.error('Failed to load hair care products');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen py-12 animate-fadeIn">
      <div className="container-custom">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4">Hair Care</h1>
          <p className="text-gray-600">
            Transform your hair with our advanced hair care formulations
          </p>
        </div>
