...
const SkinBodyCare = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // Exclude Baby Care and Hair Care categories to show only Skin & Bodycare products
        const data = await getProducts({ excludeCategories: 'Baby Care,Hair Care' });
        setProducts(data.data);
      } catch (error) {
        console.error('Error fetching skin & bodycare products:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen py-12 animate-fadeIn">
      <div className="container-custom">
        <div className="mb-8 animate-slideLeft">
          <h1 className="text-4xl font-bold mb-4">Skin & Bodycare</h1>
          <p className="text-gray-600">
            Complete skincare solutions for all your skin concerns
          </p>
        </div>
